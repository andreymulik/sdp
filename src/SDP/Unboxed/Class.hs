...
(
  ...
  -- * Unboxed
  Unboxed (..), cloneUnboxed##, concat#, sizeof#, indexof#,
...
{-# DEPRECATED fillByteArray# "in favor of fillUnboxed#, will be removed in sdp-0.4" #-}
...
class Eq e => Unboxed e
  where
    ...
    {- |
      ...
      @'indexof##' e o#@ returns the index of the element in the unboxed array
      that the offset @o#@ corresponds to (in bytes).

      @
        'indexof##' e ('sizeof##' e (8# *# i#)) === 8# *# i#

        'indexof##'       'False'   3# === 24#
        'indexof##' (0 :: 'Int32') 12# ===  3#
      @
    -}
    {-# INLINE indexof## #-}
    indexof## :: Unboxed e => Proxy# e -> Int# -> Int#
    indexof## e i# = case i# ># 0# of {1# -> (8# *# i#) `quotInt#` sizeof## e 8#; _ -> 0#}
    ...
    indexUnboxedOff# bytes# o# = let e = bytes# !# (indexof# e o#) in e
    ...
    readUnboxedOff# mbytes# o# = go proxy#
      where
        go e = \ s1# -> case (!>#) mbytes# (indexof## e o#) s1# of
          (# s2#, res #) -> (# s2#, asProxy## e res #)
    ...
    writeUnboxedOff# mbytes# o# e = writeUnboxed# mbytes# (indexof# e o#) e

    -- | Procedure for filling the array with the default value.
    fillByteArray# :: MutableByteArray# s -> Int# -> e -> State# s -> State# s
    fillByteArray# =  fillUnboxed#

    {-# INLINE fillUnboxed# #-}
    -- | Procedure for filling the array with the default value.
    fillUnboxed# :: MutableByteArray# s -> Int# -> e -> State# s -> State# s
    fillUnboxed# mbytes# n# e = I# n# > 0 ? go (n# -# 1#) $ \ s1# -> s1#
      where
        go -1# = \ s2# -> s2#
        go  c# = \ s2# -> go (c# -# 1#) (writeUnboxed# mbytes# c# e s2#)

    {-# INLINE fillUnboxedOff# #-}
    -- | Fill unboxed array by filler.
    fillUnboxedOff# :: MutableByteArray# s -> Int# -> Int# -> e -> State# s -> State# s
    fillUnboxedOff# =  defaultfillUnboxedOff#
    ...
    newUnboxed' e n# = \ s1# -> case newByteArray# (sizeof# e n#) s1# of
      (# s2#, mbytes# #) -> case fillUnboxed# mbytes# n# e s2# of
        s3# -> (# s3#, mbytes# #)
    ...
    -}
    prepareSort# :: Proxy# e -> MutableByteArray# s -> Int# -> Int# -> State# s -> State# s
    prepareSort# _ _ _ _ = \ s# -> s#

    {- |
      @since 0.3

      Restore mutable array, prepared by 'prepareSort#'.
    -}
    restoreSort# :: Proxy# e -> MutableByteArray# s -> Int# -> Int# -> State# s -> State# s
    restoreSort# _ _ _ _ = \ s# -> s#
...
instance Unboxed Int
  where
    ...
    restoreSort# _  _  0# _  = \ s1# -> s1#
    restoreSort# e bs# i# o# = \ s1# -> case readWordArray# bs# o# s1# of
      (# s2#, w# #) -> case writeWordArray# bs# o# (shiftWI# w#) s2# of
        s3# -> restoreSort# e bs# (i# -# 1#) (o# +# 1#) s3#

    {-# INLINE prepareSort# #-}
    prepareSort# = restoreSort#
    ...
    {-# INLINE indexof## #-}
    indexof## _ i# = case i# ># 0# of {1# -> quotInt# i# SIZEOF_HSWORD#; _ -> 0#}
...
instance Unboxed Int8
  where
    ...
    {-# INLINE indexof## #-}
    indexof## _ i# = case i# ># 0# of {1# -> i#; _ -> 0#}
...
instance Unboxed Int16
  where
    ...
    restoreSort# _  _  0# _  = \ s1# -> s1#
    restoreSort# e bs# i# o# = \ s1# -> case readWord16Array# bs# o# s1# of
    ...
    {-# INLINE indexof## #-}
    indexof## _ i# = case i# ># 0# of {1# -> quotInt# i# 2#; _ -> 0#}
...
instance Unboxed Int32
  where
    ...
    restoreSort# _  _  0# _  = \ s1# -> s1#
    restoreSort# e bs# i# o# = \ s1# -> case readWord32Array# bs# o# s1# of
    ...
    {-# INLINE indexof## #-}
    indexof## _ i# = case i# ># 0# of {1# -> quotInt# i# 4#; _ -> 0#}
...
instance Unboxed Int64
  where
    ...
    restoreSort# _  _  0# _  = \ s1# -> s1#
    restoreSort# e bs# i# o# = \ s1# -> case readWord64Array# bs# o# s1# of
    ...
    {-# INLINE indexof## #-}
    indexof## _ i# = case i# ># 0# of {1# -> quotInt# i# 8#; _ -> 0#}
...
instance Unboxed Float
  where
    ...
    {-# INLINE indexof## #-}
    indexof## _ i# = case i# ># 0# of {1# -> quotInt# i# SIZEOF_HSFLOAT#; _ -> 0#}
...
instance Unboxed Double
  where
    ...
    {-# INLINE indexof## #-}
    indexof## _ i# = case i# ># 0# of {1# -> quotInt# i# SIZEOF_HSDOUBLE#; _ -> 0#}
...
instance (Unboxed e, Integral e) => Unboxed (Ratio e)
  where
    ...
    restoreSort# e bs# i# o# = restoreSort# (fromGValue# e) bs# (2# *# i#) (2# *# o#)
      where
...
{\
  ...
  \
  fillUnboxed# marr# i# (Type e) = fillUnboxed# marr# i# e;\
...
instance Unboxed Bool
  where
    ...
    {-# INLINE indexof## #-}
    indexof## _ i# = case i# ># 0# of {1# -> 8# *# i#; _ -> 0#}
    ...
    fillUnboxed# bs# c# e = \ s1# -> case setByteArray# bs# 0# n# val# s1# of
    ...
    fillUnboxedOff# bs# 0# n# e = fillUnboxed# bs# n# e
    fillUnboxedOff# bs# o# n# e
        | 1# <- n# <# 1# = \ s1# -> s1#
        ...
        |      True      = \ s1# -> case readWord8Array## bs# no# s1# of
          (# s2#, w# #) ->
            ...
            in  case writeWord8Array## bs# no# val'# s2# of
                  s3# -> fillUnboxedOff# bs# (o# +# ob#) (n# -# ob#) e s3#
      where
    ...
    sortUnboxed# _ bs# n# o# = case n# <# 2# of
      ...
      _  -> \ s1# -> case boolCountM# bs# n# o# s1# of
        (# s2#, c# #) ->
          let c'# = n# -# c#
          in  case fillUnboxedOff# bs# o# c'# False s2# of
                s3# -> fillUnboxedOff# bs# (o# +# c'#) c'# True s3#
...
instance (Unboxed e, Rank1 e) => Unboxed (I1 e)
  where
    ...
    fillUnboxed#  bytes# n# (E :& e) = fillUnboxed#  bytes# n# e
...
instance (Enum e, Shape e, Bounded e, Rank1 e, Unboxed e, Shape (e' :& e), Unboxed (e' :& e))
  ...
  where
    ...
    restoreSort# e bs# n# o# = restoreSort# (fromGValue e) bs# (r# *# n#) (r# *# o#)
      where
...
instance (Enum e, Shape e, Bounded e, Rank1 e, Unboxed e) => Unboxed (T2 e)
  where
    ...
    restoreSort# e = restoreSort# (fromGValue# e)
      where
...
{- |
  ...
  Default 'fillUnboxedOff#' implementation.
-}
{-# INLINE defaultfillUnboxedOff# #-}
defaultfillUnboxedOff# :: Unboxed e => MutableByteArray# s -> Int# -> Int#
                       -> e -> State# s -> State# s
defaultfillUnboxedOff# bs# o# c# e = go# o#
  where
...
radixSortUnboxed# :: Unboxed e => Proxy# e -> MutableByteArray# s -> Int# -> Int#
                  -> State# s -> State# s
radixSortUnboxed# e bs# n# o# = \ s1# -> case restoreSort# e bs# n# o# s1# of
  s2# -> case radixSortPrepared# e bs# n# o# s2# of
    s3# -> prepareSort# e bs# n# o# s3#

radixSortPrepared# :: Unboxed e => Proxy# e -> MutableByteArray# s -> Int# -> Int#
                   -> State# s -> State# s
radixSortPrepared# e bs# n# o# = let s# = sizeof## e 1# in
...
{- |
  ...
  See 'indexof##'.
-}
indexof# :: Unboxed e => e -> Int# -> Int#
indexof# e i# = indexof## (toProxy## e) i#
...
{- |
  ...
  'setByteArray#', not 'fillUnboxedOff#' or 'fillUnboxed#'.
-}
